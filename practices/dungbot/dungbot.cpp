...
namespace lpzrobots
{
    ...
    void DungBot::makeAllLegs( const Matrix& pose, Primitive* rear, Primitive* front, Primitive* head)
    {
	    ...
	    for (int i = 0; i < LEG_POS_MAX; i++)
	    {
			...
			if( conf.testTarsusSensor)
			{
				tarsusContactSensors[ std::make_pair( LegPos(i), 0) ] = new ContactSensor(true, 65, 1.5 * conf.tarsusRadius[i%3], false, true, Color(255,255,0));
				...
				tarsusContactSensors[ std::make_pair( LegPos(i), 0) ]->init(tarsus);
			}
			...
			if( conf.testTarsus )
			{
				...
				for( int j = 1; j < 6; j++ )
				{
					 ...
					 if( conf.testTarsusSensor )
					 {
						 tarsusContactSensors[ std::make_pair( LegPos(i), j) ] = new ContactSensor(true, 65, 1.5 * radius, false, true, Color(255,0,0));
